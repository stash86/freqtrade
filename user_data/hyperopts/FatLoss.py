...
class FatLoss(IHyperOptLoss):
    ...
    def hyperopt_loss_function(
        ...
        **kwargs,
        ...
        winning_profit = winning_trades.sum()
        losing_profit = losing_trades.sum()

        avg_winning_trade = winning_trades.mean() if not winning_trades.empty else 0
        max_abs_losing_trade = losing_trades.abs().max() if not losing_trades.empty else 0
        loss_tail_ratio = (
            max_abs_losing_trade / avg_winning_trade if avg_winning_trade != 0 else 100
        )
        ...
        # skew = results["profit_abs"].skew()
        kurt_trade_dur = results["trade_duration"].kurtosis()
        ...
        ) / (math.sqrt(max(trade_duration, 1)) * max(kurt_trade_dur, 4) * max(loss_tail_ratio, 1))
